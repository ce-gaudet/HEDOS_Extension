...
def run_alc_analysis(initial_file, optimized_file):
    ...
    A = np.load(initial_file)
    B = np.load(optimized_file)

    n = A.shape[0]
    days = np.arange(DAYS + 1)

    # ------------------------------------------------------------------
    # ALC and survival models
    # ------------------------------------------------------------------
    ...
    drop40_A = ALC0 - A40
    drop40_B = ALC0 - B40
    _, p_drop = ttest_rel(drop40_B, drop40_A)

    early = slice(0, 15)
    dropA = ALC0 - ALC_A[:, early].min(1)
    dropB = ALC0 - ALC_B[:, early].min(1)
    _, p_nad = ttest_rel(dropB, dropA)
    ...
    ax1.plot(days, A_m, color="tab:blue", label="Initial")
    ax1.fill_between(days, A_m - A_e, A_m + A_e, color="tab:blue", alpha=0.2)

    ax1.plot(days, B_m, color="tab:orange", label="Optimized")
    ax1.fill_between(days, B_m - B_e, B_m + B_e, color="tab:orange", alpha=0.2)
    ...
    ax2.fill_between(days, D_m - D_e, D_m + D_e, color="lightgray", alpha=0.4)
    ...
    plt.savefig(
        os.path.join("results", "ALC_comparison.png"),
        dpi=300,
    )
